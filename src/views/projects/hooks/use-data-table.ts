...
export function useDataTable<TData, TValue>({
  ...
  // Pagination State with synchronization to URL parameters
  ...
  React.useEffect(() => {
    ...
    const newPageSize = perPage;

    if (pageIndex !== newPageIndex || pageSize !== newPageSize) {
      setPagination({ pageIndex: newPageIndex, pageSize: newPageSize });
    }
  }, [page, perPage, pageIndex, pageSize]);
  ...
  }, [sorting, page, pathname, createQueryString, router]);
  ...
  React.useEffect(() => {
    ...
    router(`${pathname}?${createQueryString(newParamsObject)}`);
  ...
  // Slicing the data to handle pagination manually
